...
bool ControlComponent::Init() {
  ...
  // SY - Add the extra needed reader for traffic law
  cyber::ReaderConfig traffic_light_reader_config;
  traffic_light_reader_config.channel_name = FLAGS_traffic_light_detection_topic;
  traffic_light_reader_config.pending_queue_size = FLAGS_traffic_light_pending_queue_size;

  traffic_light_reader_ =
      node_->CreateReader<TrafficLightDetection>(traffic_light_reader_config, nullptr);
  ACHECK(traffic_light_reader_ != nullptr);


  cyber::ReaderConfig prediction_obstacles_reader_config;
  prediction_obstacles_reader_config.channel_name = FLAGS_prediction_topic;
  prediction_obstacles_reader_config.pending_queue_size = FLAGS_prediction_pending_queue_size;

  prediction_obstacles_reader_ =
      node_->CreateReader<PredictionObstacles>(prediction_obstacles_reader_config, nullptr);
  ACHECK(prediction_obstacles_reader_ != nullptr);



  // cyber::ReaderConfig map_reader_config;
  // map_reader_config.channel_name = FLAGS_relative_map_topic;
  // map_reader_config.pending_queue_size = FLAGS_map_pending_queue_size;

  // relative_map_reader_ =
  //     node_->CreateReader<MapMsg>(map_reader_config, nullptr);
  // ACHECK(relative_map_reader_ != nullptr);
...
// SY added
void ControlComponent::OnTrafficLight(
    const std::shared_ptr<TrafficLightDetection> &traffic_light) {
  ADEBUG << "Received control data: run traffic_light message callback.";
  std::lock_guard<std::mutex> lock(mutex_);
  latest_traffic_light_.CopyFrom(*traffic_light);
}

void ControlComponent::OnPrediction(
    const std::shared_ptr<PredictionObstacles> &prediction_1) {
  ADEBUG << "Received control data: run prediction message callback.";
  std::lock_guard<std::mutex> lock(mutex_);
  latest_prediction_obstacles_.CopyFrom(*prediction_1);
}

// void ControlComponent::OnMap(
//     const std::shared_ptr<MapMsg> &map_1) {
//   ADEBUG << "Received control data: run Map message callback.";
//   std::lock_guard<std::mutex> lock(mutex_);
//   relative_map_.CopyFrom(*map_1);
// }


void ADCTrajectory::ProduceNewTrajectory(ADCTrajectory *planning_context) {
  // Todo: 1.location of the bugs 2.mutation of the trace 3. Validity Check
  return
}


void ADCTrajectory::PlanningCheckForSpec(ADCTrajectory *planning_context) {
  AINFO << "SY Module - Planning Check For Spec";

  Trace trace0;
  Spec spec0;
  trace = PrepareTraceForPlanning();
  double distance = 0;
  distance = CalculationOfDistance(trace0, spec0);

  auto threshold = 0.1;
  if (distance < threshold){
    ProduceNewTrajectory(planning_context);
  }

  // Now Ready Info:
  // 1. latest_traffic_light_: Traffic light INFO
  // 2. latest_prediction_obstacles_: Prediction of Obstacles' motion
  // 3. latest_localization_: the position, pose
  // 4. latest_chassis_: Chassis control from the preious loop 
  // 5. latest_trajectory_: Information of the planning trajetory 
  // 6. hdmap_: Map information 
  return;

}


void ControlComponent::CheckRule57(ControlCommand *control_command) {
  auto expected_signal = latest_trajectory_.decision().vehicle_signal().turn_signal();
  auto real_signal = latest_chassis_.signal().turn_signal();

  AINFO << "SY Module - is checking for rule57";

  if(expected_signal != real_signal)
  {
    AERROR << "SY Module - Rule57 is violated, Try to fix it";
    control_command->mutable_signal()->set_turn_signal(expected_signal);
    ADEBUG << "SY Module - Rule57 is fixed";
  }
  return;
}

void ControlComponent::InstantCheckForSpec(ControlCommand *control_command) {

  AINFO << "SY Module - Instant Check For Spec";

  CheckRule57(control_command);

  auto test10 = latest_traffic_light_.header().sequence_num();
  AERROR << "SY Module - Traffic light information: sequence_num: " << test10;

  auto test11 = latest_prediction_obstacles_.header().sequence_num();
  AERROR << "SY Module - Prediction obstacles information: sequence_num: " << test11;

  // auto aaa = apollo::hdmap::Id("lane_39");
  Id map_id;
  map_id.set_id("lane_39");
  auto test12 = hdmap_->GetLaneById(map_id);
  AERROR << "SY Module - Map information: lane_39: " << test12;
...
bool ControlComponent::Proc() {
  ...
  const auto &traffic_light_msg = traffic_light_reader_->GetLatestObserved();
  if (traffic_light_msg == nullptr) {
    AERROR << "traffic_light msg is not ready!";
    return false;
  }
  OnTrafficLight(traffic_light_msg);

  prediction_obstacles_reader_->Observe();
  const auto &prediction_msg = prediction_obstacles_reader_->GetLatestObserved();
  if (prediction_msg == nullptr) {
    AERROR << "prediction msg is not ready!";
    return false;
  }
  ...
  // const auto &map_msg = relative_map_reader_->GetLatestObserved();
  // if (map_msg == nullptr) {
  //   AERROR << "map msg is not ready!";
  //   // return false;
  // }
  // OnMap(map_msg);


  // Checking for the planning part and produce a new trajectory
  PlanningCheckForSpec(&latest_trajectory_);


  //Control due to the latestest message from channel
  {
  ...
  // Instant check with the current control command
  InstantCheckForSpec(&control_command);
