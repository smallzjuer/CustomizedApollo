...
// struct AtomPredicates
// {
//    char  operators[2];
//    double  LeftSignal[50];
//    double  RightSignal[50];
// };
...
{
   public:
      double LeftSignal[50];   // 长度
      double RightSignal[50];  // 宽度
      double height;   // 高度
      double get(void);
      void set( double len, double bre, double hei );
};
...
class ControlComponent final : public apollo::cyber::TimerComponent {
 ...
 private:
  ...
  // SY Added
  void OnTrafficLight(
      const std::shared_ptr<apollo::perception::TrafficLightDetection> &traffic_light);

  void OnPrediction(
      const std::shared_ptr<apollo::prediction::PredictionObstacles> &prediction_1);

  // void OnMap(
  //     const std::shared_ptr<apollo::relative_map::MapMsg> &prediction_1);


  void PlanningCheckForSpec(ADCTrajectory *planning_context);
  void ProduceNewTrajectory(ADCTrajectory *planning_context);
  void CheckRule57(ControlCommand *control_command);
  void InstantCheckForSpec(ControlCommand *control_command);
 ...
 private:
  ...
  // SY - New ones for traffic laws
  std::shared_ptr<cyber::Reader<perception::TrafficLightDetection>> traffic_light_reader_;
  perception::TrafficLightDetection latest_traffic_light_;
